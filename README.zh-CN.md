...
Actions 默认使用 `YU322142/loongarch-oldworld-sysroot` release 中的 Linux x64 旧世界 GCC 14 工具链，并下载同一 release 中的完整旧世界开发 sysroot。构建时会直接把 `--sysroot` 指向该 sysroot，不再使用公开 cross-tools 自带 sysroot 参与链接。该 sysroot 来自旧世界 Loongnix/LoongArch 开发环境，用于补齐在线构建需要的开发头文件和库。
...
- `SKIASHARP_REF=v3.119.4` 选择用于 HarfBuzzSharp 原生目标的 SkiaSharp 源码 tag。
